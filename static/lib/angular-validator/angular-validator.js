            ...
            link: function(scope, element, attrs, fn) {
                ...
                function updateValidationMessage(element) {
                    ...
                    var defaultRequiredMessage = function() {
                        return 'Wypełnij pole' // "<i class='glyphicon glyphicon-remove'></i> This field is required";
                    };
                    var defaultInvalidMessage = function() {
                        return 'Błędna wartość' //"<i class='glyphicon glyphicon-remove'></i> Incorrect value";
                    };
                    ...
                    if (scopeElementModel.$dirty || scope[element.form.name].submitted) {
                        ...
                        if (scopeElementModel.$error.required) {
                            ...
                            if ("required-message" in element.attributes) {
                                ...
                                message = element.attributes['required-message'].value;
                            }
                            ...
                            if ("invalid-message" in element.attributes) {
                                ...
                                message = element.attributes['invalid-message'].value;
                            }
                            // Display the default error message
                            else {
                                // angular.element(element).after(generateErrorMessage(defaultInvalidMessage));
                                message = defaultInvalidMessage;
                            }
                        }
                        if (message !== null) {
                            angular.element(element).after('<span class="form-error-message validationMessage"><i class="glyphicon glyphicon-triangle-top"></i> <span>' + message + '</span></span>');
                        }
                        else {
                ...
                function generateErrorMessage(messageText) {
                    return "<span class='help-block has-error validationMessage'>" + scope.$eval(messageText) + "</span>";
                }
